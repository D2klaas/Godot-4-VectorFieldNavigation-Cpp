...
#	occupied_field.dynamic = true
	...
	blocked_field = map.add_mod_field("blocked")
	blocked_field.dynamic = true
	blocked_field.boolean = true
	var occluders = get_tree().get_nodes_in_group("occluder")
	for o in occluders:
		blocked_field.add_occulder( o,1 )
...
func test():
	blocked_field.clear()
	blocked_field.refresh_occluders()
